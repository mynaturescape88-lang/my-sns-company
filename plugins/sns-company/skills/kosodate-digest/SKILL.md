...
- 範囲の開始Dayが366以上の弾では、`solve_trims.py`が出力する各segの`label`を空文字にすること。

### 3. ビルド → `build_full_28_100.py`（`build_digest.py`のランナー）
Day範囲用に**ランナーの定数を書き替えて実行**する（`build_digest.py`本体は原則触らない）：
...
- `build_digest.py` を改修したときは、改修前の仕様に該当する入力で旧実装と新実装の出力md5が一致することを実測すること（目的＝過去弾の再現性を壊さないため）。
...
- 概要欄＝**確定書式（Day101-150で確定・2026-07-11オーナー指示）**。この5ブロックをこの順・絵文字ゼロで作る：
  ...
  - 時間の書き方は、台帳にDay番号が無い動画を含むかで切り替える（目的＝Day番号が無い範囲は本数から日数・月数を確定できないため）。
  - 台帳にDay番号が無い動画を含む弾では、①1行目を範囲の最終動画時点の子の状態を述べる一文にすること（例:「1歳になったばかりの息子の記録です。」）。
  - 台帳にDay番号が無い動画を含む弾では、①3行目の時期表現を範囲の最終動画時点の月齢だけにすること（例:「1歳1ヶ月ごろの、〜」）。
  - 台帳にDay番号が無い動画を含む弾では、タイトルに書く時間の表現を範囲の最終動画時点の月齢だけにすること。
  - 台帳にDay番号が無い動画を含む弾では、タイトルを `【1歳Nヶ月】<その回の象徴イベント>｜成長記録まとめ` の型にすること。
  ...
  - 台帳にDay番号が無い動画を含む弾では、月齢タグを範囲の最終動画時点で成り立つ表記だけにすること（例:Day366-395→`#1歳` `#1歳1ヶ月`）。
  - 生後365日を超える範囲では、月齢タグを満年齢と月数で書くこと（例:`#1歳1ヶ月`）。
...
- 本編テロップ：**左上・白背景の帯枠**に**1行**。下線が**ピンク→オレンジのグラデで左から伸びる**。**auto-fitでフォント縮小し必ず1行**（長題も2行にしない）。静止表示・映像のクロスフェードに同期して切替。
  - テロップの文字列は、元ショートのタイトルにDay番号が残っているかで切り替える（目的＝Day番号の無い範囲で意味を持たない「Day N」を出さないため）。
  - 範囲の開始Dayが365以下の弾では、テロップの文字列を「`Day N  <タイトル>`」にすること。
  - 範囲の開始Dayが366以上の弾では、テロップの文字列を「`<タイトル>`」だけにすること。
