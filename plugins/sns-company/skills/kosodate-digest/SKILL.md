...
- **尺基準＝まとめる本数で決まる（恒久ルール）**：**30本まとめ＝8分以上**／**60本まとめ＝15〜20分**。
  中間の本数は本数に比例して按分する。`solve_trims.py --target-aim` 既定16.3分は60本想定の値なので、
  30本前後の弾では下限（8分）を満たしているかで判定する（上限は設けない）。日数から1日平均秒を自動配分。
- 素材の実尺合計が基準に届かない（footage-bound）ことがある。**全日ノートリムでも下限に届かない場合だけ**、
  範囲を延ばすか現尺で進めるかをオーナーへ確認する（下限を満たしていれば確認不要＝そのまま進める）。
...
- 検出されたDayは、同一弾でkeep判定されたDayとspectral flux自己相関(60-180BPM帯)のピーク値およびf0のセミトーン標準偏差を比較して楽曲かどうかを裏取りしてから、mute.jsonへの採否を決めること（目的＝賑やかな屋内環境音を楽曲と誤検出して元音の見どころを消さないため）。
- 検出器の生出力を mute_detector_raw.json として保存すること（目的＝元音ミュートの採否判断を後から検証できる証跡を残すため）。
...
- clean_title()の結果が空文字になる日は、その日のsegから telop_variant を落としてテロップ帯を出さないこと（目的＝説明部分が存在しない日に幅92pxの空白帯が焼き込まれるのを防ぐため）。
...
- テロップPNGの枚数が「日数 − テロップ帯を出さない日数」と一致することを実測すること（目的＝テロップの描画漏れと余剰を枚数で検算するため）。
- DL済み素材ファイルのmd5が全て相異なることを実測すること（目的＝同一ファイルが2つのseqへ重複して割り当てられた状態で完成させないため）。
- DL済み素材の全ペアについて内容シグネチャの距離を算出し、距離が最小のペアを実際に再生して別の映像であることを確認すること（目的＝同じ映像が別のvideoIdで再アップされた場合はmd5が一致しないため）。
- 総尺が**本数に応じた基準**を満たすか＝**30本まとめは8分以上**／**60本まとめは15-20分**（中間は按分）。30本弾で8分以上あれば総尺はpass＝15-20分に届かないことを理由にfailにしない。

### 6. 内部レビューゲート（アップ前・fail-closed・オーナー委任）
オーナー指示（2026-07-10）＝「前回の完成品と比較して問題ないと内部で確認できたらアップまで進める」。
- **前回完成品と実物比較**：帯150px・左上テロップ1行＋グラデ下線・クロスフェード0.6s・元音＋BGMバランスが同等水準か。今回の固定仕様（冒頭演出なし／鼓動なし／BGM3分割／尺は本数基準＝30本8分以上・60本15-20分／BGM-26 LUFS／末尾15秒黒）が正しく反映されているかを**実フレーム/実測**で確認。
