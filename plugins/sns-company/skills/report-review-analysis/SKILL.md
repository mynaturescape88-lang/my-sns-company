...
- ② `.company/pm/review-baseline.md`（全レビュー普遍の全23項目＝番号なしフラットのチェックリスト。「観点N」という番号は存在しない。項目は増えるので**毎回Readして数え直す**）
...
- ② `pm/review-baseline.md` の**全23項目**を1つずつ照合（1つでもfailなら提出しない＝fail-closed／特に「提案・施策が対象プラットフォームの`sns_accounts/<platform>.md`の戦略と相違していない」と「公開コンテンツのレビューでは content-compliance skill を実際に発動している」＝権利・合法性）
- ③ **fail-closed**：1つでもfailなら提出せず→修正→①へ戻る
- ④ 全passのみ次工程（マーケター戦略コメント）へ進める

## 数値分析の観点（合格ラインを内包・1つずつ照合）
- [ ] **フロー数値が「依頼時点までのリアルタイム値」で記録されているか**＝進行中の当週セル（`wk_cur=monday_of(today)`〜today）に**部分実測値**が入り、直近完了週（`wk_done=wk_cur-7`）が月〜日のフル値で確定しているか。**当週フローが空欄"-"のまま提出＝fail**（「完了週にしか書かない」旧仕様は2026-08-09オーナー訂正で廃止）。それより古い週のセルが変わっていたら＝fail（過去は不変）。週境界は暦週(月〜日)＝ローリング7日窓は不可（新規）
- [ ] **5項目構成か**＝①位置付け（狙い＋数値目標）／②戦略（実施中のみ）／③期待値（数値の想定値）／④評価（調子＋多期間＋施策ログ照合）／⑤見直し、で旧構成の残骸（独立した④観測欄・⑤評価/⑥見直しの番号ズレ）が無いか（新規）
- [ ] **④評価の現在地数値がHTMLの表セル（進行中の当週列＝部分実測／確定分は最新完了週列）と一致**するか＝別取得値とドリフトしていれば表の値へ照合したか。**進行中週の値は「M/D時点の部分実測」と明記**し、完了週と同列に前週比を語らない（旧④観測は廃止＝現在地の実値は④評価内で③と対比して述べる）（新規）
  - 表の**週列は実数値のみ**（週セルの `()` ＝前週比は2026-08-11オーナー確定で全廃）。週次WoWに触れるなら**週列の実数値どうしを自分で引く**こと。日数の違う列（月初1W＝2日・進行中の当週＝部分実測）を同列に比べていないか＝比べているなら fail
- [ ] **合計列の `()` を「前月まるごととの差」として語っていないか**＝2026-08以降の合計列の `()` は **MTD比較**＝「当月1日〜依頼時点の累計 vs 前月1日から同じ経過日数ぶんの累計」の差（セルのホバーに期間が出る）。「前月比▲◯◯だから前月を超えた」は**同経過日数どうしの比較**として述べる（月まるごとを超えたという意味にしない）。ストック（登録者/フォロワー/スキ等）だけは前月シートの月内最新値との差＝MTDではない。調子欄の「好調」も同じMTD比較が根拠。正本＝`secretary/report_format.md`「合計列の前月比＝MTD方式」（新規）
