...
- ② `.company/pm/review-baseline.md`（全レビュー普遍の各観点）
...
- ③ `pm/review-baseline.md` の各観点を1つずつ照合（pass/fail・権利・合法性（content-compliance発動確認）はfail-closed）。**戦略相違チェック観点**＝提案/施策が対象 `sns_accounts/<platform>.md` の戦略（役割・勝ち筋・コスト前提・却下レバー）と**相違していないか**を照合（相違＝fail）
