...
name: review-check
description: >
  成果物の自己チェック・PMレビューを実行するときに発動。『レビューして』『チェックして』『自己点検』『品質確認』『出す前に確認』『PMレビュー』等で発動。読むのは2つだけ＝そのタスクの fb/<task>.md ＋ pm/review-baseline.md（混在させない）。fail-closed＝全pass以外は提出せず修正、passした物だけ提示する。
---
...
- ※他タスクの fb/ は読まない（1タスク1ファイル）。深掘りが要るときだけ `[[feedback_...]]` で memory 正本へ

## フロー（発動＝この順で全実行）
- ① 対象タスクを特定 → `skill-routing.md` で該当 `fb/<task>.md` を決める（無ければ普遍のみ）
- ② `fb/<task>.md` の各「□」を成果物に1つずつ照合（pass/fail）
- ③ `pm/review-baseline.md` の6観点を1つずつ照合（pass/fail）
...
□ ③ pm/review-baseline.md の6観点を全て照合したか
