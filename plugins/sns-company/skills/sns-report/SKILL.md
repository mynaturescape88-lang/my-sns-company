...
- このスクリプトが決定的に行うこと：
  ...
  2. 週セルを `report_history.json` に記録（`record()`）＋取得可能な過去週の空セルのみ遡及充填（`backfill()`）。**過去分の再集計はしない**（`calendarize` は呼ばない＝既知の巻き戻しバグを回避。同スクリプトは2026-08-10に実行ガード付きで退役済み）。**フロー数値は「依頼時点までのリアルタイム値」で記録する**＝①**進行中の当週**（`wk_cur=monday_of(today)`〜today）の**部分実測値**を毎回**上書き**、②**直近完了週**（`wk_done=wk_cur-7`＝月〜日のフル値）を確定値で**上書き**、③**それより古い週は一切書き換えない**（不変）。同じ週でも依頼のたびに最新実測へ更新される（時点が変われば値が変わるのは仕様＝リアルタイム）。週境界は**暦週(月〜日)**＝ローリング7日窓は使わない。
     **さらに①②の各窓を暦月境界で分割し、「週∩暦月」の最大4セグメントセルを書く**（2026-08〜・`CALENDAR_MONTH_FROM`）。月跨ぎ週は「前月ぶん」「当月ぶん」の2セルへ割れ、それぞれ自分の月シートに入るので**月合計＝週セル合算が暦月の実額と一致する**。各セルは `days` / `span` を自己記述で持つ。**2026-08より前の月へは1セルも書かない**（過去シート凍結の機械的保証＝`segment_plan()` が境界前セグメントを除外）。フロー値は各取得スクリプトの `--ranges` が返す `#RANGE` ブロックからのみ取る。
...
- **暦月一致の検証**（読み取りAPIのみ・0cr）：`.venv/bin/python seo_growth/verify_calendar_month.py --ym YYYY-MM`
