...
## B. 語り部口パク動画（旧・区分④）
> ⚠️ **2026-07 語り部（案内人）廃止済＝本節は"次回デフォルト"でなく"過去動画（ナスカ第1弾／ヘルクラネウム第2弾）の再ビルド時のみ有効な参考"**。現行は The Life Guide型（ホスト非表示・純ナレーション）で語り部口パクカットを作らない。区分④は現行テンプレでは無音モーションへ振り直し済（旧④語り部→廃止／旧⑤無音→現④／旧⑥再現→現⑤）。以下の内蔵音cr浪費の教訓は**過去動画を再ビルドする際に絶対に必要なため削除せず残す**。
...
- **素材は全編ユニーク＝同一素材の2回目使用も不可（2026-07-01オーナー決定・次回デフォルト）**：症状＝第2弾ヘルクラネウム／第3弾ghost homininで、章テーマの実写prefixを行間に巡回配置する設計上、`pick_stock` が `files[i % len]` で**同一 `{prefix}_01.mp4` を1本で何度も使い回していた**（第3弾は79 stockカット中56カットが既出素材の再掲）。同じ画が繰り返し出ると単調・手抜き感が出る。→ **次回デフォルト**：①台帳・アセンブル計画の段階で**各prefixの登場回数を数え、その本数ぶん固有バリアント（`{prefix}_01,_02,…_0N`）を用意**する。②実写stockは `_stock/` に別バリアントを足すだけで `pick_stock` が自動で巡回し固有化（**アセンブラ改修不要**）。静止画jpg・合成★・図解③は別名の固有ファイルで用意（jpgは `I_(spath(...))` が固定名参照＝2本目は参照先も別名に変える）。③不足分は**追加0cr素材（Pexels/Pixabay=商用可帰属不要／Wikimedia/Archive/NASA=PD/CC）を調達してでも全カット固有素材にする**（generic stockの再掲で埋めない・AI本数は増やさない）。CC-BY/BY-SAはクレジット文を `_sources.md` に記録しエンドカード/概要欄に載せる。工程3は `jinchi-review-ledger`、工程8は `jinchi-review-assemble` の「全編ユニーク」観点で担保。[[reference_pexels_cloudflare_ua]]
- **章転換teaserの表示時間は文字数に応じた可変長に**：症状＝ヘルクラネウム版（`build_herculaneum_4k.py`）の黒画面章転換（`kind="teaser"`）は固定2.0秒。テキストは短いもので10文字・長いもので18文字あり、18文字を2.0秒で見せると秒速9文字（適正読書速度＝秒速4文字程度の倍以上）で「一瞬すぎて読めない」とオーナー指摘（2026-07-03）。→ **次回デフォルト**：teaser表示秒数は固定値でなく`文字数 ÷ 4文字/秒 + 前後の間`で算出し、下限3.0秒・上限4.5秒でクランプする（短文でも読む間を確保・長文でも間延びさせない）。第3弾以降（語り部完全廃止＝全章転換がteaser方式）から適用。既存公開済み・公開待ちのナスカ／ヘルクラネウムは遡及修正しない（2026-07-03オーナー決定）。
...
- **★台帳の尺・本数・crは"実合算"で検証してから出す（自己申告を信じない）**：症状＝第2弾ヘルクラネウムの初版台帳が「骨格カット止まり」で、A表の尺合計は約634s（10.5分）なのに集計欄は「総尺905s（15分）」と書かれ**数値が合っていなかった**（C表も全章「要敷き詰め」のまま放置）。PMレビューはcr・図解2連続・章数は裏取りしたが**尺の足し算を実検証していなかった**ため見逃した。→ **次回デフォルト**：①台帳は最初からナスカ準拠の粒度（約130カット・各章カットを実際に並べてC表で割付＝想定の差分0）で作る。「要敷き詰め」プレースホルダを残さない。②**レビュー時は機械パース等でA表の尺列を実際に合計し、章別小計・総尺・区分本数が集計欄と一致するかを必ず再計算**（特に「尺の合計／敷き詰め」はオーナーが必ず突く）。※本項の当時の「予cr／消費cr」照合はcr概念の廃止（§I）で失効＝現行は尺・本数の実合算のみ。③素材取得先はfetch_broll 6系統（動画Pexels/Pixabay/NASA/Internet Archive・画像Pexels/Pixabay/Openverse/Wikimedia Commons）＝v3で「stock無し」とした被写体もWikimedia/Archive/NASAで実写化を再検討し①を増やす（generic stock使い回し回避・AI本数は増やさない）。[[feedback_report_verify_before_reporting]]

## I. コスト前提（2026-07 Higgsfield完全解約後）
- **「1本上限300cr・0cr素材最大化」は第2弾ヘルクラネウムまでの前提＝Higgsfield完全解約（2026-07-07失効）で区分④⑤⑥等の有料AIカット自体が選択肢から消えた**。→ **台帳側の規定は `ledger-template.md` 本体へ統合済み（2026-07-17）＝cr列・cr収支節は廃止・現行区分は①②③★(間)の5種**。テンプレを見ればよく、本項で重ねて規定しない（経緯の記録として残す）。
- **本編ナレ＝Gemini TTS（Charon）・無料枠0cr・1章1リクエスト生成（章内の声の揺れをゼロ化）**。律速は金銭でなく**Gemini無料枠のRPD（日次上限・JST16時頃に回復）＋制作稼働時間**。MiniMax Audioは第2弾ヘルクラネウムまでの旧方式＝**過去動画の再ビルド時のみ参照**（新規制作の前提にしない）。コスト・契約の正本は`.company/sns_accounts/youtube_ainsight.md`コスト性質欄。
- 素材台帳の有料カット候補は、生成発火タイミングが解約日を跨ぐ可能性が高い場合は前提にせず**0cr合成/実写アーカイブで代替案を先に用意する**。
