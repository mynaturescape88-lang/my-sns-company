...
- **★素材の合法性（絶対基準・恒久）**：**使用する素材は、映像・静止画・ロゴ・公式画面スクショ・BGM・効果音・音声／ナレーション音源・フォント・その他あらゆる使用素材を問わず、選定した時点で出所とライセンスを一次規約で確認し、すべて合法であること。** 合法だと確認できない素材は使用不可＝この基準を満たさない章・成果物は完了・提示・公開しない。
  - 確認は**選定時点**（台帳に載せる時）＝事後確認は不合格。結果は台帳**§K 素材ライセンス台帳**へ全列記入する（空欄・「不明」は不合格）。
  - **固定資産（OP/ED）・使い回し・過去に承認済みの素材も対象**＝免除は無い。
  - 一次規約で確認する（サービス名・ファイル名からの推測、別サービスの規約の流用は不可）。証跡ファイルは**素材と同じディレクトリに置き、移動時は一緒に運ぶ**。
  - **判定の型＝入手経路で基準を分ける**（「確認できない」と「NGと記載されている」は別物・2026-07-20 オーナー指摘）。**第三者から取得した素材**＝許諾を積極的に確認できることが必要（出所不明は不合格）。**正規に提供された実行環境の同梱物**（OS同梱フォント等）＝**当該利用を禁止する条項が無いことを一次規約で確認できれば可**（「明示的な許諾条項が無い」ことだけを理由に不合格としない）。いずれも**確認結果と根拠＝一次規約URL＋条項番号＋禁止条項の有無を§Kに記録する**。
  - 台帳確定時に `content-compliance` skill を当てる（工程10でも再度当てる）。
...
- **★音素材（BGM/SE/ナレーション音源）も映像と同じ合法性の絶対基準（§3）の対象**＝選定時点で配布元の一次規約を確認し、台帳§Kへ `出所URL／ライセンス種別／商用可否／帰属要否／確認日／確認根拠` を記入する。**証跡ファイル（`LICENSE_AND_SOURCES.txt` 等）は音源と同じディレクトリに置き、整理・移動時は必ず一緒に運ぶ**。可能なら**機械的な同定根拠**（ID3のencoder署名・尺・ファイルハッシュ）も残す。**OP/EDに既に焼き込まれているBGMも同じ扱い**＝工程10の「§F 権利ゲート」で毎弾確認する。**判定の型は§3と同じ**＝第三者取得の音源は許諾を積極的に確認できることが必要（出所不明は不合格）／正規提供の実行環境同梱物は禁止条項が無いことを一次規約で確認できれば可。
