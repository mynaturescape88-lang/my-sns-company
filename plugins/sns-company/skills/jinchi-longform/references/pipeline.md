...
1. OP/EDの固定資産を用意する
- OP＝`op-ed/op_final_4k.mp4`、ED＝`op-ed/ed_final_4k_48k.mp4`（4Kアップスケール版）を使う。作り直さず・音量も触らない。**連結入力にするEDは必ず `_48k` 付き**＝48kHz版＝章mp4と同じ音声レートで `-c copy` 可。同ディレクトリの `ed_final_4k.mp4` は44.1kHzの**原本**で、`_48k` はここから音声だけ48k化した派生物（`<topic>_chapter_cards.py` に変換ステップを持つ弾がある＝原本は消さない）。原本をそのまま連結入力にすると音声レート不一致でstream-copyが通らない。
