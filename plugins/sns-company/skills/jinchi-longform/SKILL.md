...
3. **★章別素材台帳を作る** … `ledger-template.md`（**唯一の正本＝前作の台帳を写さない**）。再生順マスターショットリストに区分**①②③★／(間)** を割当（全カット0cr＝cr積算・300cr照合は廃止）。**使用する素材（映像・静止画・ロゴ・公式画面スクショ・BGM・効果音・音声／ナレーション音源・フォント等すべて）は、選定した時点で出所とライセンスを一次規約で確認し、すべて合法であること**（下記「絶対原則」・`references/pipeline.md`§3）＝確認結果は台帳§Kへ記録する。→ 台帳を埋めたら `content-compliance` を当ててから `jinchi-review-ledger` を発動（尺の実合算を必ず検算）。
...
1. **対象＝完成物に含まれる全素材**＝本編章の映像・図解・合成・ロゴ・公式画面 ＋ **OP/EDの映像とBGM** ＋ 章転換SE ＋ ナレーション音源 ＋ フォント。**固定資産・過去承認済み素材も対象**（免除は無い）。
2. **照合先＝台帳§K 素材ライセンス台帳**。全素材が行を持ち、`出所URL／ライセンス種別／商用可否／帰属要否／確認日／確認根拠` が全て埋まっていること。**空欄・「不明」・「—」は不合格**。
3. **確認根拠は一次規約**＝配布元サービスの規約本文（URL）＋素材と同じディレクトリに置いた証跡ファイル。**別サービスの規約を当てない**（例＝tunetank音源にPixabayの規約）。証跡ファイルが実在することを実パスで確認する。
4. **機械的な同定根拠を可能な限り残す**＝音源はID3/メタデータ（YTALならencoder=Google署名）・尺・ファイルハッシュを記録し、「この音源が確かにその配布元のその曲である」ことを示す。
5. **判定の型＝入手経路で基準を分ける**（「確認できない」と「NGと記載されている」は別物・2026-07-20 オーナー指摘）。
   - **第三者から取得した素材**＝**許諾を積極的に確認できることが必要**。出所不明は不合格。
   - **正規に提供された実行環境の同梱物**（OS同梱フォント等）＝**当該利用を禁止する条項が無いことを一次規約で確認できれば可**。「明示的な許諾条項が無い」ことだけを理由に不合格としない。
   - いずれも**確認結果と根拠（一次規約URL＋条項番号＋禁止条項の有無）を§Kに記録する**こと。
6. **出所が記録されていない素材は、画素・波形などの性質の測定で出所を代替しない**（2026-07-20 監査指摘）。例＝「彩度0だから写真ではない」は誤り＝白黒写真・グレースケール化イラストも彩度0。第三者が作った同種の物も同じ値を示す＝**画素は作者を証言しない**。**判定は保護対象要素の分解**（文言／図像／書体／音源／背景・配置）で行い、各要素が「自組織のもの」か「非保護」かを示す。性質の測定が言えるのは**物の種別の限定まで**（例＝合成レンダであって撮影物ではない）。
7. 分岐＝**1件でも満たさなければ結合しない**。素材を差し替える（OP/EDも作り直す）か、一次規約で確認し直して§Kと証跡を揃えてから再度ゲートを通す。**「オーナーがOKした」「前弾で使った」は通過理由にならない。**
...
- **素材の合法性（絶対基準・恒久）**：**使用する素材は、映像・静止画・ロゴ・公式画面スクショ・BGM・効果音・音声／ナレーション音源・フォント・その他あらゆる使用素材を問わず、選定した時点で出所とライセンスを一次規約で確認し、すべて合法であること。** 合法だと確認できない素材は使用不可＝この基準を満たさない章・成果物は完了・提示・公開しない。
  - **対象は全素材＝免除は無い**。固定資産（OP/ED）・過去弾からの使い回し・オーナーが過去に承認した素材も、毎弾この検査を通す。
  - **確認は選定時点**（素材を台帳に載せる時）に行い、§Kの素材ライセンス台帳へ記録する。**事後確認・使用後の追認は不合格**。
  - **一次規約で確認する**＝配布元サービスの規約本文を読む。ファイル名・サービス名からの推測、別サービスの規約の流用は不可（例＝tunetank音源にPixabayの規約を当てる）。
  - **判定の型＝「確認できない」と「NGと記載されている」は別物**（2026-07-20 オーナー指摘）。素材の入手経路で判定基準を分ける：
    - **第三者から取得した素材**（実写・画像・ロゴ・公式画面・音源等）＝**許諾を積極的に確認できることが必要**。出所不明・規約に辿り着けない素材は不合格＝使わない（従来どおり）。
    - **正規に提供された実行環境の同梱物**（OS同梱フォント等）＝**当該利用を禁止する条項が存在しないことを一次規約で確認できれば可**。「明示的な許諾条項が無い」ことだけを理由に不合格としない。
    - いずれの場合も**確認結果と根拠（一次規約URL＋条項番号＋禁止条項の有無）を§Kに記録する**ことは必須。記録を省く免除にはならない。
  - **ライセンス証跡ファイルは素材と同じディレクトリに置き、素材を移動・整理するときは証跡も一緒に運ぶ**（証跡だけ消えると権利を後から立証できない）。
  - **「オーナーがOKした」は権利確認の代替にならない**。「再選定・再質問しない」等の凍結は制作上の凍結であって、**権利の再検証は封じない**。
  - 発動＝`content-compliance` skill を**工程3（台帳確定時）と工程10（最終アセンブル前）で必ず当てる**。
