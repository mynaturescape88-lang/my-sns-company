...
- [ ] 手法の型（発見/解読/復元/年代測定/検出/混血検出 等）が既出弾と被っていないか。既出＝第1弾ナスカ(発見)/第2弾ヘルクラ(解読)/第3弾ghost hominin(検出)。被る場合は次のいずれかを満たさなければ不合格＝(a)よほどのインパクトのある題材、(b)台本の構成・伝え方で「また◯◯系ね」と思わせない差別化方針を工程1時点で明文化。単なる型の反復は通さない [[feedback_jinchi_topic_avoid_format_repetition]]
- [ ] 題材キューと重複・既出でないか
- [ ] ミステリー感でクリックされ誇大でないか
